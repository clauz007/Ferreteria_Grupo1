...
public class vClientes extends JFrame implements ActionListener {
	...
	public vClientes() {
		setTitle("Ferreteria Matservi - Clientes");
		...
		lblNewLabel = new JLabel("clientes", SwingConstants.CENTER);
		lblNewLabel.setFont(new Font("Segoe UI", Font.BOLD, 18));
		lblNewLabel.setForeground(COLOR_AZUL);
		lblNewLabel.setBounds(390, 15, 140, 25);
	...
	protected void do_btnAnadir_actionPerformed(ActionEvent e) {
		try {
			...
			JOptionPane.showMessageDialog(this, "Error al añadir cliente: " + ex.getMessage());
		}
	}
	...
	protected void do_btnEliminar_actionPerformed(ActionEvent e) {
		...
		int respuesta = JOptionPane.showConfirmDialog(this,
				"Desea eliminar el cliente " + txtCod.getText().trim() + "?",
				"Confirmar",
		...
		try {
			ac.eliminar(txtCod.getText().trim());
			...
			JOptionPane.showMessageDialog(this, "No se pudo eliminar. Puede tener ventas registradas.");
		}
	}

	private void listarClientes() {
		ArrayList<Cliente> lista = ac.listar();
		mostrarClientes(lista);
	}

	private void listarClientesIncompletos() {
		ArrayList<Cliente> lista = new ArrayList<Cliente>();

		try {
			Connection con = Conexion.getConexion();

			CallableStatement csta = con.prepareCall("{CALL SP_LISTAR_CLIENTE_INCOMPLETO()}");
			...
			System.out.println("Error al listar clientes incompletos: " + e);
		}
	...
	private void buscarClientes() {
		...
		try {
			...
			CallableStatement csta = con.prepareCall("{CALL SP_BUSCAR_CLIENTE_GENERAL(?,?,?)}");
			...
			System.out.println("Error al buscar clientes: " + e);
		}

		mostrarClientes(lista);
	}

	private void mostrarClientes(ArrayList<Cliente> lista) {
	...
	private String generarCodigoCliente() throws Exception {
		...
		while (rs.next()) {
			String cod = rs.getString("COD_CLI").replace("C", "");
